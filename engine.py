...
class EngineBuilder:
    ...
    def __build_engine(self,
        ...
        with self.builder.build_serialized_network(self.network, config) as engine:
...
class TRTModule(torch.nn.Module):
    ...
    def __init_engine(self) -> None:
        ...
        for i in range(num_bindings):
            if model.get_tensor_mode(names[i]) == trt.TensorIOMode.INPUT:
                num_inputs += 1
            else:
                num_outputs += 1
    ...
    def forward(self, *inputs) -> Union[Tuple, torch.Tensor]:
        ...
        for i in range(self.num_inputs):
            ...
            self.context.set_tensor_address("images",contiguous_inputs[i].data_ptr())
            if self.idynamic:
        ...
        for i in range(self.num_outputs):
            ...
            self.context.set_tensor_address(self.out_info[j-1].name,output.data_ptr())
